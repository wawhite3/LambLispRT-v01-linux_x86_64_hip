...
/*
 * LVGL 9.3.0 configuration for LambLisp.  Derived from lv_conf_template.h shipped with
 * lvgl@9.3.0, PINNED EXACTLY in w3_pio/boards.json -- a caret range fetched 9.6.0 on the first
 * build, and a baseline that moves is not a baseline.
 *
 * REACHED VIA -DLV_CONF_PATH, set by the `lvgl` feature.  IF THAT FLAG IS EVER LOST, LVGL DOES
 * NOT FAIL -- it compiles with its own defaults and says nothing, giving a working build at the
 * wrong colour depth with its heap in internal DRAM.  That is why the feature also sets
 * LV_CONF_SKIP=0 explicitly, and why this file is generated by an edit script that ASSERTS on
 * every substitution: a template rename would otherwise silently drop the setting it renamed.
 *
 * Only the settings P231 section 6 names are changed; everything else is the vendor default on
 * purpose, so a diff against the template shows exactly what we decided and nothing else.
 */
/**
 * @file lv_conf.h
...
/* clang-format off */
#if 1 /* enabled -- this is LambLisp's config, not the template */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    #define LV_MEM_SIZE (512 * 1024U)         /**< [bytes] -- in PSRAM, see pool below */
    ...
    /* P231: LVGL's heap obeys the PSRAM-first policy instead of carving its own pool out of
       internal DRAM.  The template UNDEFs these when LV_MEM_ADR==0; we define them instead. */
...
#define LV_DRAW_BUF_ALIGN                       64  /* ESP32-S3 PSRAM cache line */
...
#define LV_USE_TJPGD 1  /* the 4WD sends JPEG (P123 D1/D2); this decodes it */
 ...
 * DEMO USAGE
 ====================*/
    ...
    #define LV_USE_DEMO_WIDGETS 0  /* header-only in the PIO package: links nowhere */
