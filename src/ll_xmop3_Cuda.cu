...
#include <cublas_v2.h>
//! @defgroup xmop3_cuda CUDA GPU
//! @ingroup xmop3
//! @brief LambLisp CUDA GPU builtins.
//! @{
...
void cudaDevicePtrDestroyer(Ptr_t p) { cudaFree(p); }    //!< GC destructor for device memory
void cudaHostPtrDestroyer(Ptr_t p)   { cudaFreeHost(p); } //!< GC destructor for pinned host memory

/*! Lazily-created, process-wide cuBLAS handle (destroyed at process exit by the OS). */
static cublasHandle_t ll_cublas_handle = 0;
static cublasHandle_t ll_cublas_get(void)
{
  if (!ll_cublas_handle) cublasCreate(&ll_cublas_handle);
  return ll_cublas_handle;
}

/*! Row-major C(m x n) = A(m x k) * B(k x n) via cuBLAS, on HOST buffers.
    Our arrays are host-resident (shape . data) pairs, so we round-trip through the
    device: cudaMalloc + H2D + gemm + D2H + cudaFree.

    cuBLAS is COLUMN-major but our buffers are ROW-major.  Rather than copy/transpose,
    use the identity (A*B)^T = B^T * A^T.  A row-major (m x k) buffer, reinterpreted
    column-major, already IS A^T (k x m).  So computing, in cuBLAS column-major terms,
    Ct(n x m) = B^T(n x k) * A^T(k x m) — i.e. sgemm(N,N, n, m, k, dB, n, dA, k, dC, n) —
    writes exactly our desired row-major C(m x n) into dC with no transposes.
    Returns 0 on success, nonzero on any CUDA/cuBLAS error. */
extern "C" int array_matmul_cublas_f32(const LL_float32 *A, const LL_float32 *B, LL_float32 *C,
                                       int m, int k, int n)
{
  LL_float32 *dA = 0, *dB = 0, *dC = 0;
  size_t szA = (size_t) m * k * sizeof(LL_float32);
  size_t szB = (size_t) k * n * sizeof(LL_float32);
  size_t szC = (size_t) m * n * sizeof(LL_float32);
  if (cudaMalloc(&dA, szA) != cudaSuccess) return 1;
  if (cudaMalloc(&dB, szB) != cudaSuccess) { cudaFree(dA); return 1; }
  if (cudaMalloc(&dC, szC) != cudaSuccess) { cudaFree(dA); cudaFree(dB); return 1; }
  cudaMemcpy(dA, A, szA, cudaMemcpyHostToDevice);
  cudaMemcpy(dB, B, szB, cudaMemcpyHostToDevice);
  const LL_float32 alpha = 1.0f, beta = 0.0f;
  cublasStatus_t st = cublasSgemm(ll_cublas_get(), CUBLAS_OP_N, CUBLAS_OP_N,
                                  n, m, k, &alpha, dB, n, dA, k, &beta, dC, n);
  cudaDeviceSynchronize();
  cudaMemcpy(C, dC, szC, cudaMemcpyDeviceToHost);
  cudaFree(dA); cudaFree(dB); cudaFree(dC);
  return (st == CUBLAS_STATUS_SUCCESS) ? 0 : 1;
}

/*! Double-precision counterpart of array_matmul_cublas_f32 (uses cublasDgemm). */
extern "C" int array_matmul_cublas_f64(const LL_float64 *A, const LL_float64 *B, LL_float64 *C,
                                       int m, int k, int n)
{
  LL_float64 *dA = 0, *dB = 0, *dC = 0;
  size_t szA = (size_t) m * k * sizeof(LL_float64);
  size_t szB = (size_t) k * n * sizeof(LL_float64);
  size_t szC = (size_t) m * n * sizeof(LL_float64);
  if (cudaMalloc(&dA, szA) != cudaSuccess) return 1;
  if (cudaMalloc(&dB, szB) != cudaSuccess) { cudaFree(dA); return 1; }
  if (cudaMalloc(&dC, szC) != cudaSuccess) { cudaFree(dA); cudaFree(dB); return 1; }
  cudaMemcpy(dA, A, szA, cudaMemcpyHostToDevice);
  cudaMemcpy(dB, B, szB, cudaMemcpyHostToDevice);
  const LL_float64 alpha = 1.0, beta = 0.0;
  cublasStatus_t st = cublasDgemm(ll_cublas_get(), CUBLAS_OP_N, CUBLAS_OP_N,
                                  n, m, k, &alpha, dB, n, dA, k, &beta, dC, n);
  cudaDeviceSynchronize();
  cudaMemcpy(C, dC, szC, cudaMemcpyDeviceToHost);
  cudaFree(dA); cudaFree(dB); cudaFree(dC);
...
//! Allocate n bytes of device memory; returns a T_CPP_HEAP object with cudaFree destructor.
...
//! Allocate n bytes of page-locked host memory; returns a T_CPP_HEAP object with cudaFreeHost destructor.
...
//! Free one or more device memory objects returned by cudaMalloc.
...
//! count is in bytes.  dst and src may be device pointers (cudaMalloc), pinned host pointers (cudaMallocHost), or bytevectors.
Sexpr_t mop3_cudaMemcpy(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  ll_try {
    ...
    Sexpr_t sym_h2d = lamb.mk_symbol("cudaMemcpyHostToDevice",   env_exec);
...
//! Fill count bytes of device memory with a byte value: (cudaMemset ptr val count).
Sexpr_t mop3_cudaMemset(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  ll_try {
    ...
    cudaError_t err = cudaMemset(devPtr, val, count);
    if (err != cudaSuccess) throw NIL->mk_error("%s error %d\n", me, err);
    return OBJ_UNDEF;
  }
  ll_catch();
...
//! Create a CUDA stream; returns a T_CPP_HEAP object with cudaStreamDestroy destructor.
Sexpr_t mop3_cudaStreamCreate(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  ll_try {
    ...
    if (err != cudaSuccess) { delete s;  throw NIL->mk_error("%s error %d\n", me, err); }
...
Sexpr_t mop3_cudaStreamSynchronize(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  ll_try {
    cudaStream_t *s = (cudaStream_t *) lamb.car(sexpr)->mustbe_cppobj_t()->prechecked_cppobj_get_ptr();
