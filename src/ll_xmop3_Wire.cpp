...
Sexpr_t Wire_mop3_write(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  //! WRITE THE STRING AS AN EXPLICIT (bytes, length) PAIR.  This was
  //! `LL_Wire->write(sexpr->any_str_get_chars())`, which was CORRECT on Arduino 2.x and does not
  //! compile on 3.x: TwoWire in 2.0.17 declared its own convenience overloads --
  //!     inline size_t write(const char *s) { return write((uint8_t *) s, strlen(s)); }
  //! (Wire.h:136) plus write(long) / write(unsigned int) / etc. -- and Arduino 3.x DELETED all of
  //! them, leaving only write(uint8_t) and write(const uint8_t *, size_t).  With the char* overload
  //! gone the call binds to write(uint8_t) and fails as
  //!     invalid conversion from 'char*' to 'uint8_t' [-fpermissive]
  //! The [-fpermissive] tag makes this look like mere compiler tightening, and it is NOT: the
  //! overload it needed genuinely no longer exists.  Behaviour on 2.x was right, so this rewrite is
  //! behaviour-preserving there (identical strlen-based call) and merely explicit.
  //! Passing the pair directly is correct on BOTH cores and does not depend on convenience
  //! overloads that a future core may drop again.
  else if (sexpr->is_any_str_atom()) {
    Charst_t chars = (Charst_t) sexpr->any_str_get_chars();
    ires = (LL_int32) LL_Wire->write((const uint8_t *) chars, strlen(chars));
  }
