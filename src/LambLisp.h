...
//! THE SENTINELS ARE COMPILE-TIME CONSTANTS, AND THEY HAVE TO BE SPELLED THAT WAY.
//! These were `extern Sexpr_t NIL;` etc. -- MUTABLE variables with external linkage. The objects
//! they point at are fixed, but a variable is not a constant expression, so any other translation
//! unit writing
//!     static Sexpr_t sym = OBJ_UNDEF;      // inside a function
//! had to LOAD that variable at run time. That makes the local static DYNAMICALLY INITIALISED, and
//! the compiler must then wrap every call to the enclosing function in a thread-safe
//! initialisation guard -- __cxa_guard_acquire, which on this toolchain takes a lock. Measured at
//! the site: three such accesses in one prologue cost ~4,300 cycles PER NCG CALL, for a
//! diagnostic that was switched off.
//! Declaring the ADDRESS as constexpr fixes it at the root instead of at each call site: the
//! initialiser becomes a constant expression, the static is initialised at load time, and no guard
//! is emitted anywhere. Taking the address is a constant expression even though the Cell objects
//! themselves are constructed at startup -- the ADDRESS is known at link time, which is all this
//! needs.
//! Nothing assigns to these (checked) and nothing takes their address (checked), which is what
//! makes the conversion safe.
extern Cell _NIL_, _HASHT_, _HASHF_, _OBJ_EOF_, _OBJ_UNDEF_, _OBJ_VOID_, _OBJ_NOSLOT_, _OBJ_FREED_, _OBJ_SYSERROR_;

class DatumLabels;	//!<Per-cell colour/label table used when writing cyclic or shared structure.
...
//! Render `obj` to a String, applying datum labels according to `mode`.
...
*/
//!@{
constexpr Sexpr_t NIL = &_NIL_;		//!<The empty list.
constexpr Sexpr_t HASHT = &_HASHT_;		//!<The canonical *true* value.
constexpr Sexpr_t HASHF = &_HASHF_;		//!<The canonical *false* value.
constexpr Sexpr_t OBJ_EOF = &_OBJ_EOF_;		//!<A single EOF object is used to indicate end-of-file on all ports.
constexpr Sexpr_t OBJ_UNDEF = &_OBJ_UNDEF_;	//!<A single UNDEF object, used to initialize a Cell having no immediate default value, and as the UNBOUND-VARIABLE marker. It is **NOT** the RxRS unspecified value -- that is `OBJ_VOID`. This comment used to say it was, and `mop3_cond` believed it: an unmatched `cond` returned OBJ_UNDEF and so was indistinguishable from a failed lookup, muddying `dict_ref` diagnosis. If you are reaching for a value meaning "no result", you want OBJ_VOID.
constexpr Sexpr_t OBJ_VOID = &_OBJ_VOID_;	//!<**THE** RxRS unspecified value, and the only one. Every form that returns nothing yields THIS, on BOTH the AST and the bytecode paths, and they must never disagree: an empty body, a no-else `if` whose test failed, `when`/`unless` on the untaken branch, a `cond` with no matching clause. It is truthy, is not null, and prints as nothing. NIL and HASHF are both WRONG here and both were used before -- they are legitimate Scheme values that flow on silently, and HASHF is the worse of the two because it is Scheme's only false value, so an unspecified result acted as a NO and `(if (if #f #f) 'a 'b)` picked `'b`. The old note here ("a silent version of OBJ_UNDEF ... use with care because it will hide errors") predates /and warned against exactly the use this value now has everywhere; it is gone rather than softened. Emitted in bytecode as a LOAD_CONST, not an opcode, because BC_* and NCG_* must stay numbered in lockstep.
constexpr Sexpr_t OBJ_FREED = &_OBJ_FREED_;	//!<B154: the car of EVERY cell sitting on the free list, and nothing else. A free cell used to be built as (NIL . next-free), which is INDISTINGUISHABLE from a legitimate pair whose car is the empty list -- so a use-after-free read returned NIL and looked like ordinary data. That is exactly how B154 hid: a frame whose four (key . val) pairs had been swept reported all four KEYS as NIL, which reads as "the bindings are empty" rather than "these cells are freed". With this marker the same read prints FREED and names the fault immediately. A real static Cell, so the marker and every dump path stay safe if one is ever traversed; never bound, returned, or exposed to Scheme.
constexpr Sexpr_t OBJ_NOSLOT = &_OBJ_NOSLOT_;	//!<the T_SVEC_IMM "this slot does not exist" marker, and NOTHING ELSE.
...
constexpr Sexpr_t OBJ_SYSERROR = &_OBJ_SYSERROR_;	//!<A single instance of T_ERROR, statically allocated with a fixed size buffer (also statically allocated) for use by Cell-level errors
//!@}
...
#define LL_FLOAT_PROFILE 0  //!<Measurement build only; not enabled in a shipped runtime.
#endif
#if LL_FLOAT_PROFILE
#define LL_FLOAT_PROF_N     5
#define LL_FLOAT_PROF_TOTAL 0         //!<every mk_float32/mk_float64 call
#define LL_FLOAT_PROF_NAN  1  //!<NaN -- the ONLY value where identity and value compare disagree
...
//! classify a float the way the PROPOSED cache would, WITHOUT installing the cache.  This is
...
| These additional `pair` types are *atoms*, but list operations such as `car`, `cdr`, and `append` can operate on them.         |
  ...
  */
  //! **How to read the `(car cdr)` note on each type below.**  It names what the two slots HOLD.
  //! `Sexpr_t` means a reference to another cell, which the collector traverses.  A named scalar
  //! type -- `LL_int32`, `Charst_t`, `Bytest_t` -- means the slot holds that value directly, with
  //! no cell of its own.  `reserved` means the slot is unused.  `wide` means the value is 8 bytes
  //! and occupies BOTH slots where a machine word is 4 bytes.  `slot-typed` means the slot may be
  //! either: an embedded scalar or a reference, decided per slot at run time and recorded in the
  //! cell's flags -- ask `car_is_ptr()` / `car_is_embedded()` rather than assuming.
  //!
  //! A `T_PAIR` is slot-typed too: `cons` packs a small integer or float straight into the slot
  //! instead of allocating a cell for it, and `Lamb::car()` / `Lamb::cdr()` build the number back
  //! when you read it.  This is invisible from Scheme and matters in C++ only if you reach past
  //! those accessors into the raw words.
  //!@{
  enum {
    //Complex atoms first
    ...
    T_STR_HEAP,		//!<(LL_int32 Charst_t)	car=BYTE length, cdr=ptr to a payload allocated len+1 and ZEROED -- i.e. EXACTLY the shape of T_BVEC_HEAP above, which is the physical half of "a string is a bytevector" (B317). CORRECTED 2026-09-09: this said the car "may be used in future to store the string length". It already does, and has at every site that builds or resizes one -- ll_vm_mk.cpp:57, ll_vm_mop3_rxrs.cpp:4262 and :4297. What is missing is not the field, it is a READER: any_str_get_length() still does strlen(), so mop3_string_length scans the buffer twice while the correct byte length sits right here. The CHARACTER length is the one genuinely absent.
    ...
    T_STR_EXT,		//!<(reserved Charst_t)	Same as T_STR_HEAP but externally allocated; the character array is not freed at GC time. UNLIKE T_STR_HEAP the car really is unused here -- _OBJ_SYS_STR_ (ll_vm_cell.cpp:23) is built with car=0 -- so an external string's length comes from strlen and nowhere else (B317).
    ...
    T_FLOAT64,		//!<(LL_float64 wide)       Inexact 64-bit LL_float64; 8-byte value at &car (spans car+cdr on ESP32).  GC-safe: type < T_NIL.
    ...
    */
    T_NIL,		//!<(ERROR ERROR) NIL has its own type and a singleton instance.
      //! The type order is meaningful.  Every type ordered ABOVE `T_NIL` carries two slots that the
      //! garbage collector traverses; every type ordered at or above `T_PAIR` is a pair to Scheme.
      //! `T_COMPLEX` and `T_RATIONAL` sit between the two: their parts are traced by the collector,
      //! but they remain NUMBERS to your program, so `(car 1/2)` raises an error rather than
      //! returning 1 -- the same answer a full Scheme such as Chez gives.
      // Maintainer note, deliberately not published: moving these two above T_PAIR was tried and
      // reverted -- it makes (car 1/2) answer 1, and Cell::features[] in ll_vm_cell.cpp is
      // positional, so an enum reorder without a matching table reorder reads the wrong rows.
    T_COMPLEX,		//!<(slot-typed) Complex number atom; car=real part, cdr=imaginary part.  EACH SLOT is an embedded LL_int32/LL_float32 OR a reference to another number cell, so the collector DOES traverse the slots -- a complex is an atom to your program but not to the collector.
  T_RATIONAL,		//!<(slot-typed) Exact rational atom; car=numerator, cdr=denominator (>=2, GCD-reduced).  EACH SLOT is an embedded LL_int32 OR a traced pointer to a boxed T_INT32/T_INT64/T_BIGNUM -- so ALONE among the types below T_PAIR, the GC DOES scan it; see the T_RATIONAL case in mark_one_quantum().
    ...
    */
    T_PAIR,		//!<(slot Sexpr_t) Normal cons cell.  EACH SLOT holds a reference to another cell OR an embedded LL_int32 or LL_float32 -- `cons` packs small numbers directly into the slot rather than allocating a cell for them, and Lamb::car()/cdr() materialise them again on the way out.  Types below T_PAIR are atoms.
  ...
  /*! @name THE SUBTYPE BITS -- flags bits 4-7 (`flags & 0xF0`).

    ```
      bit   7   6   5   4   3   2   1   0
          +---+---+---+---+---+---+---+---+
          |  subtype bits |TAI|  gc state |
          +---+---+---+---+---+---+---+---+
             0xF0          0x08   0x07
    ```

    Four bits whose meaning is decided by the cell's top-level type.  TODAY ONLY PAIRS USE THEM,
    as the four booleans below: is the car an embedded int, is it an embedded float, and the same
    two for the cdr.  Other types are expected to use these bits soon; when the second user
    arrives the shared part can be factored out THEN, from two real cases rather than from one
    imagined one.  There is deliberately no generic subtype accessor yet.

    WHY THERE IS NO `Wt_` ENUM ANY MORE.  These were a 2-bit field per slot with an enum
    `WordType { Wt_ptr=0, Wt_int32=1, Wt_float32=2, Wt_reserved=3 }`, and the constants were named
    `F_CAR_WT0` / `F_CAR_WT1` for "car word-type bit 0/1".  That model was more complicated than
    the facts: `Wt_reserved` never occurs, and `Wt_ptr` is not a stored value at all -- it is the
    ABSENCE of both embedded bits.  The names made it worse, because `F_CAR_WT0` reads as the
    VALUE `Wt_ptr=0` when it means BIT 0.  Four named booleans say the same thing with no enum, no
    shift, and no unreachable fourth state.

    A SLOT WITH NEITHER BIT SET HOLDS A `Cell*` THAT THE COLLECTOR TRACES.  That is the invariant
    the marker and the write barrier depend on, and it is why `tcons()` zeroing a recycled cell is
    correct by construction: a fresh cell's slots start as traced pointers.

    THE VALUES ARE BAKED INTO GENERATED MACHINE CODE.  The four NCG backends emit literal tests
    against 0x10 and 0x40 -- ll_vm_ncg_x86_64.cpp:1116, ll_vm_ncg_aarch64.cpp:880 (`tbnz` bit 4),
    ll_vm_ncg_xtensa.cpp:843, ll_vm_ncg_rv32.cpp:717.  Changing a VALUE here silently changes what
    already-compiled code tests; changing a NAME does not.
  */
  //!@{
  static const int F_SUBTYPE_MASK = 0xF0;	//!<The subtype bits. Meaning is per top-level type; today only pairs use them.

  static const int F_CAR_IS_INT   = 0x10;	//!<PAIR subtype: car holds an embedded LL_int32 (not a traced pointer).
  static const int F_CAR_IS_FLOAT = 0x20;	//!<PAIR subtype: car holds an embedded LL_float32 (not a traced pointer).
  static const int F_CDR_IS_INT   = 0x40;	//!<PAIR subtype: cdr holds an embedded LL_int32 (not a traced pointer).
  static const int F_CDR_IS_FLOAT = 0x80;	//!<PAIR subtype: cdr holds an embedded LL_float32 (not a traced pointer).
  //!@}
...
#define _byte2_ptr_	(&(_contents._byte[2]))
  ...
  */
  ...
  void init_static_data();
    ...
    //! The type order is meaningful: see the note in the type enumeration above.
  //! True when the cell is an ATOM -- anything that is not a pair.  Numbers, symbols, characters,
  //! strings, bytevectors and the empty list are atoms.  So are exact rationals and complex numbers:
  //! the garbage collector traverses their internal parts, but your program sees each as a single
  //! numeric value, so `car` and `cdr` reject them as they would any other number.
  Bool_t is_atom(void)			{ return _type_ < T_PAIR; }
  ...
  //! True for every cell whose car and cdr hold Scheme values -- not just `T_PAIR` but procedures,
  //! dictionaries, thunks, errors, bytecode and the rest of the pair region.  This is the question
  //! `car` and `cdr` ask.  It is NOT the collector's question: see `is_slot_typed()`.
  Bool_t is_any_pair(void)		{ return _type_ >= T_PAIR; }
  ...
  //! B351: does this cell's VALUE denote an integer?  This is what R7RS `integer?` asks, and it is
  //! NOT is_any_int(): exactness is a different question, so (integer? 2.0) is #t.  THE SINGLE
  //! IMPLEMENTATION for all three execution paths -- mop3_integer_q (interpreter), BC_INTEGER_P
  //! (bytecode) and ncg_integer_p (native).  They each had their own answer and all three
  //! disagreed; keep it that way and they will drift again.  Defined in ll_vm_mop3_rxrs.cpp,
  //! beside the interpreter entry point it was extracted from.
  Bool_t is_integer_valued(void);
  ...
  //! @name PAIR subtype accessors -- the four subtype bits as pairs use them.
  //! Meaningful for T_PAIR and the slot-typed types (T_RATIONAL, T_COMPLEX) only.  When another
  //! type starts using the subtype bits it will name its own predicates for them; do not reach
  //! for these there.
  //!@{
  /*! @brief True when the collector traverses this cell's two slots.

    ASK THIS, NOT `is_any_pair()`, WHEN YOU ARE REASONING ABOUT THE COLLECTOR.  The two questions
    look alike and are not the same:

      - `is_any_pair()` asks whether *Scheme* sees a pair -- what `car` and `cdr` will accept.
      - `is_slot_typed()` asks whether the *garbage collector* follows what is in the two slots.

    Exact rationals and complex numbers answer `#f` to the first and `true` to the second: your
    program sees a single number, while the collector traces the parts it is built from.  If you
    write a mop3 that stores a cell reference into a slot, this is the predicate that tells you
    whether that reference will be traced.
  */
  bool is_slot_typed()  const  { return _type_ > T_NIL; }
  bool car_is_int()      const  { return (_flags_ & F_CAR_IS_INT)   != 0; }			//!<True when the car slot holds an embedded LL_int32.
  bool car_is_float()    const  { return (_flags_ & F_CAR_IS_FLOAT) != 0; }			//!<True when the car slot holds an embedded LL_float32.
  bool cdr_is_int()      const  { return (_flags_ & F_CDR_IS_INT)   != 0; }			//!<True when the cdr slot holds an embedded LL_int32.
  bool cdr_is_float()    const  { return (_flags_ & F_CDR_IS_FLOAT) != 0; }			//!<True when the cdr slot holds an embedded LL_float32.
  bool car_is_embedded() const  { return (_flags_ & (F_CAR_IS_INT | F_CAR_IS_FLOAT)) != 0; }	//!<True when the car holds an embedded value of either kind (one bitwise op).
  bool cdr_is_embedded() const  { return (_flags_ & (F_CDR_IS_INT | F_CDR_IS_FLOAT)) != 0; }	//!<True when the cdr holds an embedded value of either kind (one bitwise op).
  //! A slot with NEITHER embedded bit set holds a Cell pointer the collector traces.  "Is a
  //! pointer" is the absence of the two bits, not a value stored anywhere -- which is why a
  //! zeroed cell is correct by construction.
  bool car_is_ptr()      const  { return !car_is_embedded(); }
  bool cdr_is_ptr()      const  { return !cdr_is_embedded(); }
  //!@}

  //! REMOVED 2026-09-11 -- `is_any_embedded()`, which was
  //!     bool is_any_embedded() const { return (_flags_ & 0xF0) != 0; }
  //! It tested the WHOLE subtype nibble with NO TYPE GUARD and answered "a car or cdr holds an
  //! embedded value".  That reading is only true for T_PAIR and the slot-typed types.  It had NO
  //! CALLERS, which is the only reason it was ever harmless -- and the moment any other type
  //! stores a subtype value it would have started answering that about, say, a string.  If a
  //! whole-nibble test is ever wanted again, write it with a type guard, or use `subtype()` and
  //! compare against the value THAT type defines.  Deleted rather than fixed because a predicate
  //! whose name is right for exactly one type is a trap regardless of its body.
  LL_int32    car_int32()   const  { return (LL_int32)    _car_; }				//!<Car word as a signed int32.  ONLY meaningful when `car_is_int()`; on any other slot this returns the bit pattern of whatever is there, including a pointer.
  LL_float32  car_float32() const  { return *(LL_float32*) &_car_; }				//!<Car word as a float32.  ONLY meaningful when `car_is_float()`.
  LL_int32    cdr_int32()   const  { return (LL_int32)    _cdr_; }				//!<Cdr word as a signed int32.  Same caution as `car_int32()`.
  LL_float32  cdr_float32() const  { return *(LL_float32*) &_cdr_; }				//!<Cdr word as a float32.  Same caution as `car_float32()`.
  //!@}
  ...
  LL_int32   gc_state(void)	{ return _flags_ & GC_STATE_MASK; }							//!<Return the garbage-collection state of this cell.
  ...
  LL_int32   tail_state(void)	{ return _flags_ & F_TAIL; }								//!<Return the tail-call state of this cell.
  ...
  /*! @par Reading a cell's value -- four prefixes, and the difference matters

    The accessors come in four families.  Choosing the wrong one is the commonest way to write a
    mop3 that works on your bench and fails on a customer's data.

      `as_X()`          Reads the bits and TRUSTS YOU.  No type check.  If the cell is not the type
                        you assumed, you get a meaningless value rather than an error -- an integer
                        made from a pointer, a float made from a length.  Use it only where the type
                        is already established.
      `mustbe_X()`      CHECKS the type and raises a LambLisp error if it is wrong.  This is what a
                        procedure should call on its arguments: the error names the offending value
                        and reaches the user as a normal Scheme error rather than a crash.
      `prechecked_X()`  Unchecked, and named so you can see at a glance that the check was done
                        elsewhere -- typically by a `mustbe_` or a predicate a line or two earlier.
      `any_X_get_*()`   The safe compound: checks, then reads.  Usually what you want.

    The rule of thumb: validate arguments with `mustbe_`, then work with `prechecked_` or the raw
    accessors.  Reach for `as_` only inside code that has just tested the type itself.
  */
  ...
  */
  ...
  Sexpr_t set(LL_int32 typ, LL_int32 a,  Sexpr_t b)	{ _type_ = typ;  _car_ = (Word_t) a;  _cdr_ = (Word_t) b;  return this; }	//!<Convenience: car is a raw (sign-extended) int32.  ATOMS only -- never a pair type (a pair car must be a full pointer; see the Word_t/Sexpr_t overload below, added for ).
  Sexpr_t set(LL_int32 typ, Word_t a, Sexpr_t b)	{ _type_ = typ;  _car_ = a;  _cdr_ = (Word_t) b;  return this; }	//!<a (Word_t,Sexpr_t) call formerly resolved to the (LL_int32,Sexpr_t) overload and SIGN-TRUNCATED the car (e.g. expand()'s `set(T_PAIR,(Word_t)NIL,free_list_head)` stored (LL_int32)NIL).  This full-width overload wins that resolution.
  ...
  Sexpr_t set(LL_float64 d)			{ _type_ = T_FLOAT64; _car_ = _cdr_ = 0; memcpy(&_car_, &d, 8); return this; }	//!<Set cell as T_FLOAT64 (8 bytes at &car; spans cdr on ESP32)
      //! A complex number's two slots each hold either an EMBEDDED int32/float32 or a REFERENCE to
      //! another number cell, and the collector traces the references.  If you add a setter that
      //! stores a reference, it must clear that slot's word-type bits so the slot is marked as a
      //! reference -- `rat_set_car_ptr()` is the model.  A slot whose word type still says
      //! "embedded" is never traced, and its target is collected while you are still using it.
    //! SETTING THE WORD TYPES IS PART OF WRITING A SLOT, not an optimisation.  A recycled cell
    //! arrives with its slots marked as references, so a setter that writes a raw value and leaves
    //! the word type alone hands the collector a float bit pattern to follow as if it were an
    //! address.  Set the word type in the same expression that writes the value, as the setters
    //! here and the T_RATIONAL setter below both do.  The low nibble carries GC state and is
    //! preserved.
  Sexpr_t set(LL_float32 r, LL_float32 i)	{ _type_ = T_COMPLEX; _car_ = _cdr_ = 0;
    memcpy(&_car_, &r, 4); memcpy(&_cdr_, &i, 4);
    _flags_ = (_flags_ & 0x0F) | F_CAR_IS_FLOAT | F_CDR_IS_FLOAT;   //!< both slots an embedded float32 (bit1 set, bit0 clear)
    return this; }	//!<Set cell as T_COMPLEX (single precision)
  ...
  // car=numerator, cdr=denominator; each slot is either an EMBEDDED int32 (an embedded int32, common small
  // case, no allocation, not GC-scanned) or a traced pointer to a boxed exact integer (T_INT32/T_INT64/
  ...
  Sexpr_t set(LL_int32 num, LL_int32 den) {
    ...
    _flags_ = (_flags_ & 0x0F) | F_CAR_IS_INT | F_CDR_IS_INT;   //!< both slots an embedded int32 (preserve gc_state/F_TAIL)
    return this;
  }
  void rat_set_car_i32(LL_int32 v) { _car_ = 0; memcpy(&_car_, &v, 4); _flags_ = (_flags_ & ~(F_CAR_IS_INT|F_CAR_IS_FLOAT)) | F_CAR_IS_INT; }  //!< embedded int32 numerator (embedded)
  //! RAW SLOT WRITE -- the embed-vs-link DECISION belongs to the caller, and both callers make it:
  //! Lamb::rat_store_i64() embeds when the value fits int32, and Lamb::rat_store_cell() embeds a
  //! T_INT32.  Do not add a fits-in-int32 test here; it would duplicate that decision in a third
  //! place and the three would drift.
  void rat_set_car_ptr(Sexpr_t p)  { _car_ = (Word_t) p;              _flags_ =  _flags_ & ~(F_CAR_IS_INT|F_CAR_IS_FLOAT);              }  //!< boxed-int numerator ptr (a traced pointer)
  void rat_set_cdr_i32(LL_int32 v) { _cdr_ = 0; memcpy(&_cdr_, &v, 4); _flags_ = (_flags_ & ~(F_CDR_IS_INT|F_CDR_IS_FLOAT)) | F_CDR_IS_INT; }  //!< embedded int32 denominator (embedded)
  void rat_set_cdr_ptr(Sexpr_t p)  { _cdr_ = (Word_t) p;              _flags_ =  _flags_ & ~(F_CDR_IS_INT|F_CDR_IS_FLOAT);              }  //!< boxed-int denominator ptr (a traced pointer)
  LL_int64 int_as_i64() const { return _type_ == T_INT64 ? as_int64() : (LL_int64) as_int32(); }   //!< value of a boxed T_INT32/T_INT64 (NOT T_BIGNUM)
  Sexpr_t rat_car_ptr() const { return (Sexpr_t) _car_; }   //!< boxed numerator cell   (valid when car_is_ptr())
  Sexpr_t rat_cdr_ptr() const { return (Sexpr_t) _cdr_; }   //!< boxed denominator cell (valid when cdr_is_ptr())
  bool rat_car_big() const { return car_is_ptr() && ((Cell*) _car_)->type() == T_BIGNUM; }
  bool rat_cdr_big() const { return cdr_is_ptr() && ((Cell*) _cdr_)->type() == T_BIGNUM; }
  ...
  LL_int64 as_numerator() const   { return car_is_int() ? (LL_int64) car_int32() : ((const Cell*) _car_)->int_as_i64(); }  //!<Numerator as int64 (guard rat_is_big()).
  LL_int64 as_denominator() const { return cdr_is_int() ? (LL_int64) cdr_int32() : ((const Cell*) _cdr_)->int_as_i64(); }  //!<Denominator as int64 (guard rat_is_big()).
  ...
  int exact_sign() {
    switch (_type_) {
      ...
      case T_RATIONAL:
        if (car_is_int()) { LL_int32 v = car_int32(); return v > 0 ? 1 : v < 0 ? -1 : 0; }
  ...
  Sexpr_t mk_error(const char *fmt, ...) CHECKPRINTF_pos2;			//!<Fill and return OBJ_SYSERROR, the ONE statically allocated error cell.  Allocates nothing, so it still works when the heap cannot -- but only one exists, so a second call overwrites the first.  For an error you may hold alongside others, use Lamb::mk_error().
  Sexpr_t mk_error(Sexpr_t irritants, const char *fmt, ...) CHECKPRINTF_pos3;	//!<As above, with an irritants list.  Same single shared cell.
  ...
  */
  ...
  /*! @name T_STR_HEAP payload block -- B317.

    The cdr points at a BLOCK, not at the characters:

        block:  [ LL_int32 character length ][ k bytes of UTF-8 ][ NUL ]
                ^ base == cdr                 ^ chars == base + LL_STR_HDR

    The BYTE length lives in the car, exactly as a bytevector keeps its own, so the zero-copy byte
    view of a string reads it from the cell with no dereference.  The CHARACTER length lives at the
    head of the block because it is a property of the CONTENT: a grow or shrink rewrites it inside
    the block it is already touching, and a realloc carries it along, so there is no second place
    to forget.  Both are full LL_int32 -- there is no ceiling on either.

    PRINTING IS A KNOWN, ACCEPTED DEVIATION -- B317 Phase 5, and it is NOT a bug to re-file.
    A string may CONTAIN a NUL: both lengths are stored, so string-length, string-ref, substring and
    the byte view all report the truth.  But the payload is kept NUL-TERMINATED so a string can still
    be handed to C as a `char *` at no cost, and every consumer of that pointer stops at the first
    NUL.  So such a string reports its true length and PRINTS SHORT -- through Cell::str(), display,
    write, the text of an error message, and any %%s logging.  That is the deliberate trade for a
    C-compatible buffer, not an oversight, and the manual records it too ("A string may contain NUL,
    and string-length is the authority").  If you are here because a string measured 6 and printed 3,
    this is why; the defect would be a length that lies, and it does not.

    FREE THE CDR, NEVER THE CHARS.  `get_chars()` returns an INTERIOR pointer now.  Handing it to
    ll_payload_free() frees base+LL_STR_HDR, which is heap corruption with no symptom at the call
    site.  Every free path takes `get_cdr()` -- the raw slot, the pointer payload_alloc actually
    returned -- so there is no accessor in a free path that could return an interior pointer.
  */
  //!@{
  static const int LL_STR_HDR = (int) sizeof(LL_int32);	//!<Bytes of block header before the characters.
  Ptr_t     prechecked_str_heap_get_block()				{ return as_Ptr_t(); }			//!<The BLOCK base == the cdr == what payload_alloc returned.  This is the only thing that may be freed.
  CharVec_t prechecked_str_heap_get_chars()				{ return (CharVec_t) ((char *) as_Ptr_t() + LL_STR_HDR); }	//!<The characters, LL_STR_HDR bytes into the block.  INTERIOR POINTER -- never free this.
  LL_int32  prechecked_str_heap_get_charlength()			{ LL_int32 n; memcpy(&n, as_Ptr_t(), sizeof(n)); return n; }	//!<Character count, read from the block head.
  void      prechecked_str_heap_set_charlength(LL_int32 n)		{ memcpy(as_Ptr_t(), &n, sizeof(n)); }				//!<Write the character count into the block head.
  //!@}
  ...
  //! THIS MUST COPY.  `str()` returns a String BY VALUE, so the old one-liner
  ...
  //! Size: a gensym renders as "G" followed by hex digits, at most 18 bytes on a
...
#define THROW_BAD_TYPE { throw mk_error("%s Bad type %s", me, dump().c_str()); }
  ...
  else if (typ == T_RATIONAL) {  //!< a component may be a boxed bignum -> coerce it via the boxed cell
      LL_float64 n = (car_is_int()) ? (LL_float64) car_int32() : ((Cell*) _car_)->coerce_float64();
      LL_float64 d = (cdr_is_int()) ? (LL_float64) cdr_int32() : ((Cell*) _cdr_)->coerce_float64();
  ...
  */
  ...
  //! THESE TWO ARE THE WHOLE OF B317, AND THEY ARE WHERE ITS FIX LANDS.
  //! Both derive the length from the TERMINATOR, which is why a LambLisp string cannot contain a
  //! NUL: writing a 0 into the middle does not set a byte, it SHORTENS THE OBJECT and strands the
  //! tail.  Five B263 guards exist solely to hold that invariant up.
  //! THE BYTE LENGTH IS ALREADY STORED AND ALREADY MAINTAINED -- T_STR_HEAP keeps it in the car at
  //! all three sites that build or resize one (ll_vm_mk.cpp:57, ll_vm_mop3_rxrs.cpp:4266, :4301).
  //! NOTHING READS IT.  So mop3_string_length computes a strlen here, DISCARDS it, and then walks
  //! the same buffer again with ll_u8_strlen -- two full scans for a number sitting in the car.
  //! Pointing these at the car for T_STR_HEAP (byte 2 for T_STR_IMM, strlen for T_STR_EXT, whose
  //! car really is 0) is the cheap win and is behaviour-preserving WHILE the B263 guards stand.
  //! ORDER MATTERS AND IT IS EASY TO GET BACKWARDS: today a stray NUL makes strlen return SHORT
  //! (the string reads truncated); once these read a stored length the same corruption returns the
  //! FULL length and callers read PAST the NUL.  So the guards must come down AFTER every consumer
  //! stops deriving length from the terminator, never in the same change.  Full plan in B317.
  //! BYTE length.  T_STR_HEAP keeps it in the car (B317) -- that field was always maintained and
  //! never read, so mop3_string_length used to compute a strlen here and discard it.  T_STR_IMM is
  //! bounded by imm_limit so a scan is bounded by construction; T_STR_EXT is a static literal whose
  //! car really is 0, and which mutation already refuses, so it can never contain a NUL either.
  LL_int32 any_str_get_length() {
    if (_type_ == T_STR_HEAP) return (LL_int32) _car_;
    return (LL_int32) strlen(any_str_get_chars());
  }
  void any_str_get_info(LL_int32 &len, CharVec_t &chars) {
    chars = any_str_get_chars();
    len   = (_type_ == T_STR_HEAP) ? (LL_int32) _car_ : (LL_int32) strlen(chars);
  }
  //! CHARACTER length -- what R7RS `string-length` returns.  Stored for a heap string; counted for
  //! the other two, where the buffer is bounded by the cell.
  /*! @brief Mark a heap string's cached character count UNKNOWN.  No-op for anything else.

    CALL THIS AFTER ANY WRITE THAT REACHES A STRING'S BYTES WITHOUT RESIZING IT.  The resize path
    (ll_str_splice) already invalidates, because it is the only way the BYTE length can change -- but
    the CHARACTER count can change with the byte count fixed.  Writing one UTF-8 lead byte over an
    ASCII byte merges it with its successor: 26 bytes stay 26 bytes and become 25 characters.
    The byte view of a string (B47) is exactly such a path: `bytevector-u8-set!` and
    `bytevector-copy!` accept a string and write its bytes raw, with no UTF-8 validation and without
    going through any string entry point.  P177 predicted this precisely -- "a stale flag is a WRONG
    ANSWER, not a slow one" -- and it was measured true before this existed: after
    `(bytevector-u8-set! s 0 206)` on a 26-char ASCII string, `string-length` still said 26.
  */
  void str_invalidate_charlength() {
    if (_type_ == T_STR_HEAP) prechecked_str_heap_set_charlength(0);
  }

  //! A STORED 0 MEANS "NOT KNOWN", AND THAT POLARITY IS DELIBERATE.  ll_str_splice() only makes
  //! ROOM -- its callers write the new bytes AFTER it returns -- so the count cannot be computed
  //! there; the buffer is not final yet.  Every resize therefore stores 0 and the count is
  //! recomputed on the next ask and cached here.
  //! Choosing 0 rather than -1 as the sentinel makes the failure mode free in both directions: a
  //! freshly payload_alloc'd block is memzero'd, and a construction path that forgets to write the
  //! header gets a RECOMPUTE (slow, correct) instead of a silent 0-length string.  The one value it
  //! collides with is a genuinely empty string, whose recount is a scan of zero bytes.
  //! A missed update costs speed, never correctness -- the same rule as the GC state bits.
  LL_int32 any_str_get_charlength() {
    if (_type_ != T_STR_HEAP) return ll_u8_strlen(any_str_get_chars());
    LL_int32 n = prechecked_str_heap_get_charlength();
    if (n == 0) {
      n = ll_u8_strlen_n(prechecked_str_heap_get_chars(), (LL_int32) _car_);
      prechecked_str_heap_set_charlength(n);
    }
    return n;
  }
  //!@}
  ...
  */
  ...
  void any_svec_get_info(LL_int32 &Nelems, Sexpr_t *&elems) {
    ...
    if (typ <= T_SVEC2N_HEAP) {
      ...
      Nelems = 2;	//assume
  if (prechecked_anypair_get_cdr() == OBJ_NOSLOT) Nelems--;	//!<NOT OBJ_VOID -- that is a real value
  ...
  else if (is_any_str_atom()) Nelems = any_str_get_length();  //!< BYTE length of a string
    else THROW_BAD_TYPE;
  ...
  */
  ...
  else if (is_any_str_atom()) res = (ByteVec_t) any_str_get_chars();  //!< byte view of a string
    else THROW_BAD_TYPE;
    return res;
  }
  ...
  else if (is_any_str_atom()) {  //!< byte view of a string -- see the note above
      CharVec_t s;
  ...
  //! @name Array element-type tag -- HEAP AND EXT SHAPES ONLY.  NOT VALID ON T_BVEC_IMM.
  //! These read and write Cell byte 3, which is free on T_BVEC_HEAP / T_BVEC_EXT (car holds the
  //! length, cdr the payload pointer) -- but on T_BVEC_IMM BYTE 3 IS ELEMENT 0.  The immediate
  //! layout is [0]=type [1]=flags [2]=element count [3..]=elements, so calling these on an
  //! immediate READS A DIMENSION BYTE AS A TYPE, or OVERWRITES the first element with one.  There
  //! is no type check here to stop you: `_contents._byte[3]` is taken unconditionally.
  //! CORRECTED 2026-09-09 -- this said "valid for all bvec subtypes", which is false and is the
  //! opposite of the truth for the one subtype where it matters.
  //! A PLAIN BYTEVECTOR HAS NO ELEMENT TYPE AT ALL.  mk_bytevector() sets no tag; an R7RS
  //! bytevector is just bytes.  This tag belongs to the ARRAY feature (ll_vm_mop3_extra.cpp),
  //! where an array is a SHAPE bytevector plus a DATA bytevector: the shape carries the element
  //! type and the dims, the data is untagged.  `array-type` reads it from the shape.
  //! HOW THE ARRAY CODE HANDLES THE IMMEDIATE CASE, and copy it if you add a caller: it does NOT
  //! use these accessors.  When the shape fits immediate (`1 + 4*ndim <= sizeof(Cell) - 3`) it
  //! stores the type as sdata[0] -- element 0 of the payload -- and puts the dims at sdata+1,
  //! sizing the vector with that leading byte included.  Only the heap branch calls
  //! bvec_set_elem_type().  See ll_vm_mop3_extra.cpp:533, which is the whole convention.
  //! Element types are Cell::T_INT32, T_INT64, T_FLOAT32, T_FLOAT64.
  LL_int32  bvec_elem_type(void) const { return _contents._byte[3]; }			//!<Get the array element-type tag from Cell byte 3.  HEAP/EXT SHAPES ONLY -- on T_BVEC_IMM this is element 0, not a type.
  void      bvec_set_elem_type(LL_int32 t) { _contents._byte[3] = (Byte_t)t; }		//!<Set the array element-type tag in Cell byte 3.  HEAP/EXT SHAPES ONLY -- on T_BVEC_IMM this OVERWRITES element 0.
  //!@}
...
struct NcgFrame;       //!< Native-code-generator call frame; opaque to library users.

struct NcgErrJmp;      //!< JIT exception-escape context; opaque to library users (ll_vm_internal.h).
...
//! (see the garbage-collection rules for C++ procedures):
...
//! the freshly-consed arg spine mid-call and the mop3 reads a freed cell (a trivial builtin
...
//! recurred: the fix landed at some sites and was missed at others).
...
public:
  ...
  //!< first allocation (arg cells are otherwise unrooted and can be swept mid-call).  To build a
  ...
  ~Lamb();		//!<Real teardown: finalize every cell, free the blocks and the memory
			//!<manager.  Pairs with `new Lamb` + setup().  Before
  ...
  */
  ...
  /*! @par Why every constructor takes an `env_exec`

    It is easy to read `mk_int32(7, env_exec)` and wonder what making an integer has to do with an
    environment.  The answer is garbage collection, and it is worth knowing before you write a
    procedure that allocates.

    Any allocation may run a slice of the collector before it returns.  The `env_exec` you pass is
    handed to that collection as a ROOT, so the environment -- and everything reachable from it,
    which is most of your program's live data -- is guaranteed to survive the collection your own
    allocation just triggered.  Pass the `env_exec` your procedure was called with; do not pass
    `NIL` to save a parameter, and do not invent one.

    This protects the ENVIRONMENT.  It does not protect intermediate values you are building on the
    C stack -- see the garbage-collection rules for C++ procedures in the manual, and
    `mop3_gc_protect`.
  */
  ...
  LL_int32 mem_dbg_urgent_bulk();
  unsigned long mem_dbg_t_gc_total_us();   //!< monotonic total us in gc_pass -- measured GC load
  LL_int32 mem_dbg_nfree();		//!<free cells right now (instantaneous)
  LL_int32 mem_dbg_total_cells();	//!<total cells in the pool right now
  ...
  //! are the REPL echo and the `(quiet)` / `(verbose)` procedures.
  // Maintainer note: declared here, defined beside the ll_term singleton that stores it -- a shipped
  // ll_xmop3_*.cpp must be
  // Maintainer note: a shipped ll_xmop3_*.cpp must be able to set this without including a header
  // that is stripped from customer packages -- doing otherwise made every package unbuildable.
  ...
  void  ncg_frame_pool_free();  //!< release them; nothing did, so every instance leaked the pool.
  //!@}
  ...
  */
  ...
  Sexpr_t dict_append_bang(Sexpr_t d1, Sexpr_t d2, Sexpr_t env_exec);			//!<Append d2 as a parent dictionary of d1.
  ...
  */
  ...
  //!< the current in/out/error ports are ordinary global bindings in the base env (which is
  ...
  */
  //!@{
  Sexpr_t mk_error(Sexpr_t env_exec, Sexpr_t irritants, const char *fmt, ...) CHECKPRINTF_pos4;	//!<Allocate a NEW T_ERROR cell with these irritants.  Unlike Cell::mk_error(), which reuses one shared object, several of these can be live at once.  This is the one to raise from a mop3.
  Sexpr_t mk_error(Sexpr_t env_exec, const char *fmt, ...) CHECKPRINTF_pos3;			//!<Allocate a NEW T_ERROR cell.  See the irritants overload above.
  Sexpr_t mk_syserror(const char *fmt, ...) CHECKPRINTF_pos2;					//!<Fill and return the SHARED system error object (OBJ_SYSERROR).  Allocates nothing; only one exists at a time.
  ...
  Sexpr_t mk_syserror(Sexpr_t irritants, const char *fmt, ...) CHECKPRINTF_pos3;		//!<As above, with irritants.  Still the shared object.
  //!@}
  ...
  */
  ...
  // Maintainer note: an alloc_cell() returning a zeroed UNTYPED cell so the caller writes it once
  // was tried and measured.  All three were CORRECT (rxrs 443/0, extend 629/0, conform 1337/0, and
  ...
  //! for small values.  That asymmetry is an embedded float read through car/cdr materializes
  //! a FRESH cell every time, so `(eq? x x)` on a NaN can differ between the interpreter and the
  //! compiler.  Compare floats with `=`, not `eq?`.
  // Maintainer note: a float cache was proposed; the branch is paid by the misses too, and measurement
  ...
  //! den==1, else a T_RATIONAL whose components box to T_INT64 when they overflow int32 (-- no
  ...
  Sexpr_t mk_rational(LL_int64 num, LL_int64 den, Sexpr_t env_exec) {
    ...
    c->set((LL_int32) 0, (LL_int32) 1);                        //!< establish T_RATIONAL + an embedded int32 flags
  ...
  //! Big-rational support: components may be arbitrary-precision integers.
  ...
  Sexpr_t mk_number(Charst_t str, Sexpr_t env_exec);		//!<Parse `str` as a number; returns `T_INT32`/`T_INT64`, `T_FLOAT32`/`T_FLOAT64`, `T_RATIONAL`, or `T_COMPLEX` as appropriate, or `OBJ_UNDEF` if `str` is not a valid numeric literal.
  ...
  //! [B536] exact compare of two exact integers of ANY width; <0 / 0 / >0.  Use this rather than
  //! coercing to double -- a double holds 53 bits, so two exact integers differing by 1 compared
  //! EQUAL above that, in both `=` and `<`.
  int     exact_int_cmp(Sexpr_t a, Sexpr_t b, Sexpr_t env_exec);
  ...
  */
  //!@{
#if LL_NETWORK
  Sexpr_t mk_udp_port(unsigned local_port, Sexpr_t env_exec);	//!<B404: open a UDP datagram socket.
  Sexpr_t mk_tcp_client_port(Charst_t host, unsigned port, Sexpr_t env_exec);	//!<Open a TCP connection to `host:port`; return a bidirectional port. Scheme: `(open-tcp-client-port host port)`. Returns #f on connection failure (B474 -- it did NOT throw, and did not report at all).
  ...
  */
  ...
  //! TUNABLE FROM SCHEME: `(recursion-limit)` / `(recursion-limit BYTES)` reads and sets
  //! eval_stack_budget_ (mop3_recursion_limit, ll_vm_mop3_extra.cpp), clamped at
  //! LL_EVAL_STACK_BUDGET_MIN.  Lowering it makes this guard trip at shallow depth, which is how
  //! ll_tests/bug-tests.scm (b70-*) asserts B70 in ~1 ms instead of ~20 s and ~39 k frames.
  //! IF YOU CHANGE THE CHECK BELOW, RUN THAT TEST: a regression here is not a FAIL line, it is a
  //! SIGSEGV that ends the run, so the suite's declared-total check is what reports it.
  //!@{
...
#if LL_ESP32
  void diag_memory_report();  //!< dump GC-buffer / cell-block placement (INTERNAL vs PSRAM) + sizes.
#endif
...
private:
  ...
  Sexpr_t _r5_cur_in_cell;  //!< cached (%current-input-port . port) binding pair in the base env; port lives in the (permanently-rooted) env, traced normally -- no special GC root.
  ...
  LL_int32 produce_token(LL_Port &src, Sexpr_t &env_exec);  //!< env_exec: #; must READ and discard a datum
  ...
  //Loop-based control
  Sexpr_t sym_loop;
  Sexpr_t sym_loop_deadline_ms;	//!< `loop-deadline-ms` -- the frame ceiling the idle-GC donation gates on
  Sexpr_t sym_gc_idle_strategy;	//!< `gc-idle-strategy` -- which idle-GC pacing strategy to use
  ...
  LL_int32 *rd_lab_ids;        //!<Label id per registered datum label; grown on demand.
...
// Maintainer note: gc_protect idiom for the inline Lamb members below.  ll_gc_protect is
// mem->rootpush-based and is NOT in scope here -- this header is included first in
...
inline Sexpr_t Lamb::car(Sexpr_t c)	{
  ...
  if (c->type() >= Cell::T_PAIR) {
    if (c->car_is_embedded()) {
      if (c->car_is_int())   return mk_int32(c->car_int32(), NIL);
      if (c->car_is_float()) return mk_float32(c->car_float32(), NIL);
    }
...
inline Sexpr_t Lamb::prechecked_anypair_get_cdr(Sexpr_t c)	{
  if (c->cdr_is_embedded()) {
    if (c->cdr_is_int())   return mk_int32(c->cdr_int32(), NIL);
    if (c->cdr_is_float()) return mk_float32(c->cdr_float32(), NIL);
  }
