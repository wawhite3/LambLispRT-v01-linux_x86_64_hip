 ...
 * Date		2024-02-29
 ...
 Modifed white3@frobeniusnorm.com to accomodate fwd/back compatibility withh esp rmt.
...
//Some fwd/back compatibility

#ifdef WS2812_compatibility

#define RMT_MEM_NUM_BLOCKS_1 RMT_MEM_64
#define RMT_MEM_NUM_BLOCKS_2 RMT_MEM_128
#define RMT_MEM_NUM_BLOCKS_3 RMT_MEM_192
#define RMT_MEM_NUM_BLOCKS_4 RMT_MEM_256
#define RMT_MEM_NUM_BLOCKS_5 RMT_MEM_320
#define RMT_MEM_NUM_BLOCKS_6 RMT_MEM_384
#define RMT_MEM_NUM_BLOCKS_7 RMT_MEM_448
#define RMT_MEM_NUM_BLOCKS_8 RMT_MEM_512

#define RMT_WAIT_FOR_EVER SPINLOCK_WAIT_FOREVER

#endif

WS2812::WS2812(uint16_t n /*= 8*/, uint8_t pin_gpio /*= 2*/, uint8_t chn /*= 0*/, LED_TYPE t /*= TYPE_GRB*/)
{
	ledCounts = n;
	pin = pin_gpio;
	rmt_chn = chn;
...
void WS2812::setLedCount(uint16_t n)
{
	ledCounts = n;
#ifdef WS2812_compatibility
...
void WS2812::setLedType(LED_TYPE t)
{
	rOffset = (t >> 4) & 0x03;
	gOffset = (t >> 2) & 0x03;
	bOffset = t & 0x03;
...
esp_err_t WS2812::setLedColorData(int index, uint32_t rgb)
{
	return setLedColorData(index, rgb >> 16, rgb >> 8, rgb);
}

esp_err_t WS2812::setLedColorData(int index, uint8_t r, uint8_t g, uint8_t b)
{
	uint8_t p[3];
	p[rOffset] = r * br / 255;
	p[gOffset] = g * br / 255;
...
esp_err_t WS2812::set_pixel(int index, uint8_t r, uint8_t g, uint8_t b)
{
...
esp_err_t WS2812::setLedColor(int index, uint32_t rgb)
{
	return setLedColor(index, rgb >> 16, rgb >> 8, rgb);
}

esp_err_t WS2812::setLedColor(int index, uint8_t r, uint8_t g, uint8_t b)
{
	setLedColorData(index, r, g, b);
	return show();
}

esp_err_t WS2812::setAllLedsColorData(uint32_t rgb)
{
	for (int i = 0; i < ledCounts; i++)
	{
...
esp_err_t WS2812::setAllLedsColorData(uint8_t r, uint8_t g, uint8_t b)
{
	for (int i = 0; i < ledCounts; i++)
	{
...
esp_err_t WS2812::setAllLedsColor(uint32_t rgb)
{
...
esp_err_t WS2812::setAllLedsColor(uint8_t r, uint8_t g, uint8_t b)
{
...
uint32_t WS2812::Wheel(byte pos)
{
